...
function navbar(){
    ...
    var path="/";
    var nb = 1;
    content+="<td class='btnimg'  onclick=\"currentpath='/'; SendFileCommand('list','all');\">/</td>";
...
if (String(jsonresponse.files[i].size) != "-1")
    ...
    content +="<TD class='btnimg' style=\"padding:0px;\"><a href=\"/SD"+jsonresponse.path+jsonresponse.files[i].name+"\" target=_blank><div class=\"blacklink\">";
...
if (confirm("Confirm deletion of file: " + filename))SendFileCommand("delete",filename);
}
function Deletedir(filename){
if (confirm("Confirm deletion of directory: " + filename))SendFileCommand("deletedir",filename);
}
function Createdir(){
